...
echo -e "正在安装nginx-mtproxy... "
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -p 29565:80 -p $port:443 ellermister/nginx-mtproxy:latest
        ;;
    esac
...
# echo -e "输入 docker logs nginx-mtproxy 获取链接信息"
    ...
    echo -e "TG认证地址：http://${public_ip}:29565/add.php"
